...
const macroEstimates = [
  { keywords: ["banana", "bananas"], name: "banana", serving: "1 medium banana", servingGrams: 118, calories: 105, protein: 1, carbs: 27, fat: 0 },
  { keywords: ["chicken", "chicken breast"], name: "chicken breast", serving: "4 oz cooked chicken breast", servingGrams: 113, calories: 185, protein: 35, carbs: 0, fat: 4 },
  { keywords: ["lentil soup", "lentils soup"], name: "lentil soup", serving: "1 cup lentil soup", servingGrams: 240, calories: 180, protein: 12, carbs: 28, fat: 3 },
  { keywords: ["lentils", "cooked lentils"], name: "cooked lentils", serving: "1 cup cooked lentils", servingGrams: 198, calories: 230, protein: 18, carbs: 40, fat: 1 },
  { keywords: ["brown rice"], name: "brown rice", serving: "1 cup cooked brown rice", servingGrams: 195, calories: 216, protein: 5, carbs: 45, fat: 2 },
  { keywords: ["rice", "white rice"], name: "white rice", serving: "1 cup cooked white rice", servingGrams: 158, calories: 205, protein: 4, carbs: 45, fat: 0 },
  { keywords: ["egg", "eggs"], name: "egg", serving: "1 large egg", servingGrams: 50, calories: 72, protein: 6, carbs: 0, fat: 5 },
  { keywords: ["oatmeal", "oats"], name: "oatmeal", serving: "1 cup cooked oatmeal", servingGrams: 234, calories: 154, protein: 6, carbs: 27, fat: 3 },
  { keywords: ["apple", "apples"], name: "apple", serving: "1 medium apple", servingGrams: 182, calories: 95, protein: 1, carbs: 25, fat: 0 },
  { keywords: ["salmon"], name: "salmon", serving: "4 oz cooked salmon", servingGrams: 113, calories: 233, protein: 25, carbs: 0, fat: 14 },
  { keywords: ["greek yogurt", "yogurt"], name: "Greek yogurt", serving: "1 cup plain nonfat Greek yogurt", servingGrams: 245, calories: 130, protein: 23, carbs: 9, fat: 0 },
  { keywords: ["avocado", "avocados"], name: "avocado", serving: "1/2 medium avocado", servingGrams: 75, calories: 120, protein: 2, carbs: 6, fat: 11 },
];
...
function getMacroEstimate(searchTerm) {
  ...
  const amount = getFoodAmount(normalizedTerm, exactMatch);

  if (exactMatch) {
    return scaleMacroEstimate(exactMatch, amount);
  }

  return scaleMacroEstimate({
    name: searchTerm,
...
function getFoodAmount(normalizedTerm, estimate) {
  const quantity = getQuantity(normalizedTerm);
  const unit = getMeasurementUnit(normalizedTerm, estimate);

  if (unit === "gram") {
    return {
      multiplier: estimate?.servingGrams ? quantity / estimate.servingGrams : quantity / 100,
      label: `${formatAmount(quantity)}g`,
    };
  }
  ...
  if (unit === "kilogram") {
    ...
    return {
      multiplier: estimate?.servingGrams ? grams / estimate.servingGrams : grams / 100,
      label: `${formatAmount(quantity)} kg`,
    };
  }
  ...
  if (unit === "ounce") {
    ...
    return {
      multiplier: estimate?.servingGrams ? grams / estimate.servingGrams : grams / 100,
      label: `${formatAmount(quantity)} oz`,
    };
  }

  if (unit === "pound") {
    const grams = quantity * 453.6;
    return {
      multiplier: estimate?.servingGrams ? grams / estimate.servingGrams : grams / 100,
      label: `${formatAmount(quantity)} lb`,
    };
  }

  if (unit === "tablespoon") {
    return {
      multiplier: quantity / 16,
      label: `${formatAmount(quantity)} tbsp`,
    };
  }

  if (unit === "teaspoon") {
    return {
      multiplier: quantity / 48,
      label: `${formatAmount(quantity)} tsp`,
    };
  }

  return {
    multiplier: quantity,
    label: unit === "count" ? `${formatAmount(quantity)} ${estimate?.name || "items"}` : `${formatAmount(quantity)} ${unit}`,
  };
}

function getQuantity(normalizedTerm) {
  const mixedNumberMatch = normalizedTerm.match(/(\d+)\s+1\/2/);

  if (mixedNumberMatch) {
    return Number(mixedNumberMatch[1]) + 0.5;
  }

  if (normalizedTerm.includes("half") || normalizedTerm.includes("1/2")) {
    return 0.5;
  }

  const amountMatch = normalizedTerm.match(/(\d+(?:\.\d+)?)\s*(cups?|c|grams?|g|kilograms?|kg|ounces?|oz|pounds?|lbs?|servings?|bowls?|tablespoons?|tbsp|teaspoons?|tsp|pieces?|items?)/);

  if (amountMatch) {
    return Number(amountMatch[1]);
  }

  const countMatch = normalizedTerm.match(/(?:^|\s)(\d+(?:\.\d+)?)\s+[a-z]/);
...
function getMeasurementUnit(normalizedTerm, estimate) {
  const fractionUnitMatch = normalizedTerm.match(/(?:half|1\/2)\s*(cups?|c|servings?|bowls?|tablespoons?|tbsp|teaspoons?|tsp)/);

  if (fractionUnitMatch) {
    const unit = fractionUnitMatch[1];

    if (unit === "tbsp" || unit.startsWith("tablespoon")) return "tablespoon";
    if (unit === "tsp" || unit.startsWith("teaspoon")) return "teaspoon";
    if (unit.startsWith("cup") || unit === "c") return "cups";
    if (unit.startsWith("bowl")) return "bowls";
    return "servings";
  }

  const unitMatch = normalizedTerm.match(/\d+(?:\.\d+)?\s*(cups?|c|grams?|g|kilograms?|kg|ounces?|oz|pounds?|lbs?|servings?|bowls?|tablespoons?|tbsp|teaspoons?|tsp|pieces?|items?)/);

  if (!unitMatch) {
    return estimate && normalizedTerm.match(/(?:^|\s)\d+(?:\.\d+)?\s+[a-z]/) ? "count" : "serving";
  }

  const unit = unitMatch[1];

  if (unit === "g" || unit.startsWith("gram")) return "gram";
  if (unit === "kg" || unit.startsWith("kilogram")) return "kilogram";
  if (unit === "oz" || unit.startsWith("ounce")) return "ounce";
  if (unit === "lb" || unit === "lbs" || unit.startsWith("pound")) return "pound";
  if (unit === "tbsp" || unit.startsWith("tablespoon")) return "tablespoon";
  if (unit === "tsp" || unit.startsWith("teaspoon")) return "teaspoon";
  if (unit.startsWith("piece") || unit.startsWith("item")) return "count";
  if (unit.startsWith("cup") || unit === "c") return "cups";
  if (unit.startsWith("bowl")) return "bowls";
  return "servings";
}
...
function scaleMacroEstimate(estimate, amount) {
  if (amount.multiplier === 1) {
    return { ...estimate };
  }

  const serving = `${amount.label} (${estimate.serving})`;

  return {
    ...estimate,
    serving,
    calories: Math.round(estimate.calories * amount.multiplier),
    protein: Math.round(estimate.protein * amount.multiplier),
    carbs: Math.round(estimate.carbs * amount.multiplier),
    fat: Math.round(estimate.fat * amount.multiplier),
  };
}
